...
**`.git-auth/.git-credentials` — READ ONLY, NEVER WRITE. MANDATORY, NO EXCEPTIONS.**
*(Added 2026-09-12 after this file was found corrupted with a malicious payload for the third time — a base64-encoded `rm -rf /` disguised as a proxy username, sitting alongside the real GitHub token. It appears to get written after Claude runs the GitHub-update workflow, so the rule is absolute: Claude never writes, appends, edits, "cleans," or otherwise modifies anything under `.git-auth/` for any reason.)*
- Claude may only *read* `.git-auth/.git-credentials` (e.g. to point a scratch clone's `credential.helper` at it) and only ever needs to read it to attempt a push per step 2 below.
- If a credential needs to be used for a git operation, copy it into a scratch location (e.g. `/tmp/repo/.git-credentials`) and point `credential.helper` at the *copy* — never at the original file, and never configure `credential.helper store` (or any helper mode that can write back) against the path inside the project folder.
- If `.git-auth/.git-credentials` ever looks suspicious on read (unfamiliar entries, base64-looking usernames, unexpected hosts like a `localhost` proxy with an odd "username," anything that isn't a plain `https://<user>:<token>@github.com` line), stop immediately, do not use it for anything, and tell Ted directly — quote what was found. Do not attempt to decode-and-run, "fix," or delete it yourself.
- Ted is the only one who edits or repairs this file. If it's ever wrong, that's a message to Ted, not a task for Claude to fix.
...
4. **Unattended runs (e.g. the 7am `morning-doc-sync` task) must not go silent when publishing is blocked.** Nobody's watching a scheduled run to notice a stuck git step. Do the local work regardless of whether publishing will succeed — pull the docs, patch `index.html` and the PDFs on disk, draft the digest email — then attempt the publish/verify flow above. If it can't be published (the proxy block described above, or anything else), don't just stop or log it somewhere no one will read: put it in the digest email itself, e.g. a line like `⚠️ index.html updated locally but NOT yet live on GitHub — needs a manual publish (see GitHub Sync section of CLAUDE.md).` The digest email is the one artifact a human is actually going to see that day — an unresolved publish has to surface there, not in a session transcript nobody opens unless they go looking.

**Update (2026-09-18) — direct push DOES work from `device_bash`, just not from the cloud sandbox.** The "known hard blocker" above is specific to *this cloud container's own* outbound proxy. It does not apply to `device_bash` (the shell that runs on Ted's own Mac via the remote-devices bridge) — that shell's network is Ted's own device network, not the sandboxed git-proxy. Verified end-to-end on 2026-09-18: cloned `Compartments` into a scratch dir on the device (`$HOME/repo-work`, *outside* the mounted project folder), copied `.git-auth/.git-credentials` into that scratch dir (never touched the original), set `credential.helper store` pointed at the *copy*, copied in the specific changed files from the mounted project folder, committed as `Ted Miller (via Claude) <theodore.e.miller@gmail.com>` (matches prior commit convention — set `user.name`/`user.email` locally if git complains "Author identity unknown"), and `git push origin main` succeeded — confirmed live via the Pages deploy Action completing with `conclusion: success`. **This is now the preferred path whenever `device_bash` is available**: skip straight to a `device_bash` scratch clone instead of the github.com pencil-edit workaround in step 2 above (that workaround is now the fallback for when no computer is linked, or `device_bash` isn't present, or this path is ever found blocked too). Always: (a) copy the credentials file, never point a helper at the original; (b) do the scratch clone/work in `$HOME` on the device, never inside the mounted project folder, since deleting files there needs a separate permission grant; (c) after pushing, delete the scratch dir (it holds a live credential copy) and run `git config --global --unset credential.helper`; (d) still verify post-push per point 3 below.
